namespace sample3
{
    ...
    internal class Program
    {
            ...
            monster = new List<Monster>
        {
          ...
          new Monster("슬라임", 1, 10, 3, 30, 30 ,"일반", 0, 0, 50),
          new Monster("고블린", 2, 13, 6, 50, 50, "일반", 0, 0, 70),
          new Monster("오크", 3, 20, 8, 80, 80, "일반", 0, 0, 150),
          new Monster("악어", 3, 20, 8, 40, 40, "일반", 0, 0, 100)
        };
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (currentFloor >= 1)
                {
                    Console.WriteLine($"[{i + 1}] {selectedMonster.Name} (Lv.{selectedMonster.Level}) - " +
                        $"공격력: {selectedMonster.Atk + (selectedMonster.Atk * (currentFloor - 1) * 0.2)}, " +
                        $"방어력: {selectedMonster.Def + (selectedMonster.Def * (currentFloor - 1) * 0.2)}, " +
                        $"체력: {selectedMonster.Hp + (selectedMonster.Hp * (currentFloor - 1) * 0.2)}/{selectedMonster.MaxHp + (selectedMonster.MaxHp * (currentFloor - 1) * 0.2)}, " +
        ...
        static void Battle(Monster monster, List<Monster> allMonsters)
        {
            ...
            while (player.Hp > 0 && monster.Hp > 0)
            {
                ...
                switch (playerChiose)
                {
                    ...
                    case 2:
                        ...
                        {
                            Console.WriteLine("스킬포인트가 부족합니다. 스킬을 사용할수 없습니다.");
                        }
                ...
                if (monster.Bleeding == true && monster.BleedingCnt < 3)//출혈데미지 출력
                {
                    monster.Hp -= 5;
                    monster.BleedingCnt++;
                    Console.WriteLine($"출혈로 인해 {monster.Name}에게 5의 데미지를 주었습니다.");
                    Console.WriteLine($"{monster.Name} 체력 {monster.Hp}");

                }
                else if (monster.BleedingCnt >= 3)
                {
                    monster.BleedingCnt = 0;
                    monster.Bleeding = false;
                }
                if (monster.Stun == true && monster.StunCnt < 1)
                {
            ...
            else
            {
                ...
                Console.WriteLine($"{monster.Reward}골드를 획득했습니다.");
    ...
    public class Monster
    {
        ...
        public int BleedingCnt { get; set; }
        public bool Stun { get; set; }
        public int StunCnt { get; set; }
        public int Reward { get; set; }
        public Monster(string name, int level, int atk, int def, int hp, int maxhp, string monsterClass, int bleedingCnt, int stunCnt, int reward)
        {
            Name = name;
            Level = level;
            Atk = atk;
            ...
            Bleeding = false;//특정 공격을 맞으면 활성화
            Stun = false;//일정 데미지 이상 맞으면 활성화
            BleedingCnt = bleedingCnt;
            StunCnt = stunCnt;
            Reward = reward;
        }
