namespace sample3
{
    ...
    internal class Program
    {
            ...
            monster = new List<Monster>
        {
          ...
          new Monster("슬라임", 1, 10, 3, 30, 30 ,"일반", 10),
          new Monster("고블린", 2, 13, 6, 50, 50, "일반",20),
          new Monster("오크", 3, 20, 8, 80, 80, "일반",40),
          new Monster("악어", 3, 20, 8, 40, 40, "일반",30)
        };
        ...
        static void Incount()
        {
            ...
            for (int i = 0; i < numberMonsters; i++)
            {
                ...
                if (currentFloor > 1)
                    Console.WriteLine($"[{i + 1}] {selectedMonster.Name} (Lv.{selectedMonster.Level}) - 공격력: {selectedMonster.Atk + (int)(selectedMonster.Atk * currentFloor * 0.2)}, 방어력: {selectedMonster.Def + (int)(selectedMonster.Def * currentFloor * 0.2)}, 체력: {selectedMonster.Hp + (int)(selectedMonster.Hp * currentFloor * 0.2)}/{selectedMonster.MaxHp + (int)(selectedMonster.MaxHp * currentFloor * 0.2)}, 계급: {selectedMonster.Class}");
        ...
        static void Battle(Monster monster, List<Monster> allMonsters)
        {
            ...
            else
            {
                ...
                if (allMonsters.All(m => m.IsDead))
                {
                    ...
                    Console.WriteLine("");
                    Console.WriteLine($"마리를 처치하셨습니다 !");
                    Console.WriteLine("");
                    Console.WriteLine($"Lv.{player.Level} {player.Name}({player.Job})");
                    Console.WriteLine($"HP : {player.Hp}");
                    Console.WriteLine("");
                    Console.WriteLine("[획득 아이템]");
                    Console.WriteLine($"{monster.Gold}G 획득하셨습니다.");
                    player.Gold += monster.Gold;
                    Console.WriteLine("");
                    Console.WriteLine($"보유 골드 : {player.Gold}G");
                    Console.WriteLine("0. 돌아가기");
                    Console.WriteLine("1. 올라가기");
    ...
    public class Monster
    {
        ...
        public Monster(string name, int level, int atk, int def, int hp, int maxhp, string monsterClass, int gold)
        {
            Name = name;
            Level = level;
            Atk = atk;
